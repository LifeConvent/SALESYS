...
$(function () {
    ...
    $('#bx_nb_define').attr('class','active');
    ...
    $('#form_date1').datetimepicker({
        ...
        $('#daily_report2').bootstrapTable('refresh', {url: HOST + "index.php/Home/BxWorkDefine/getNbDefine?queryDateStart="+$('#dtp_input2').val()+"&queryDateEnd="+$('#dtp_input3').val()});
    });
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#daily_report2').bootstrapTable({
            url: HOST + "index.php/Home/BxWorkDefine/getNbDefine",   //请求后台的URL（*）
            method: 'get',      //请求方式（*）
...
function actionFormatter_review(value, row, index){
    ...
    if(is_reviewer == 0){
        ...
        return '<button type="button" class="btn btn-primary pass_right" style="height: 20pt;width: 80pt"><span style="margin-left:-5pt;">通过-系统正确</span></button>' +
            ...
            '<button type="button" class="btn btn-success no_count_finish" style="height: 20pt;width: 100pt;margin-top: 5pt;margin-left: 5pt"><span style="margin-left:-2pt;">不计入已操作基数</span></button>';
    }
...
window.actionEvents_review = {
    'click .no_pass': function (e, value, row, index) {
        ...
        $.ajax({
            ...
            url: HOST + "index.php/Home/BxWorkDefine/updateReason", //目标地址.
            dataType: "json", //数据格式:JSON
            data: {username: username, is_pass:'0',business_code: business_code, policy_code: policy_code, business_node:business_node,insert_date:busi_insert_date,no_pass_reason:no_pass_reason},
            success: function (result) {
                if (result.status == 'success') {
                    debugger;
                    //单行刷新数据
    ...
    'click .pass_error': function (e, value, row, index) {
        ...
        $.ajax({
            ...
            dataType: "json", //数据格式:JSON
            //针对于未提交缺陷的系统错误
            data: {username: username, is_pass:'1',business_code: business_code, policy_code: policy_code, business_node:business_node,insert_date:busi_insert_date,result:result,
    ...
    'click .no_count_finish': function (e, value, row, index) {
        var business_node = row.business_node;
        var business_code = row.old_policy_code;
        var policy_code = row.old_policy_code;
        var busi_insert_date = row.busi_insert_date;
