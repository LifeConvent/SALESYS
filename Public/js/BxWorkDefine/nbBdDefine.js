...
$(function () {
    ...
    $('#form_date1').datetimepicker({
        ...
        $('#daily_report2').bootstrapTable('refresh', {url: HOST + "index.php/Home/BxWorkDefine/getNbBdDefine?queryDateStart="+$('#dtp_input2').val()+"&queryDateEnd="+$('#dtp_input3').val()});
    });
...
function exportExcel() {
    window.location.href = HOST + "index.php/Home/BxWorkDefine/expNbBd";
}
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#daily_report2').bootstrapTable({
            url: HOST + "index.php/Home/BxWorkDefine/getNbBdDefine",   //请求后台的URL（*）
            method: 'get',      //请求方式（*）
            showExport: true,
            exportDataType: 'all',
...
window.actionEvents_sure = {
    'click .sure': function (e, value, row, index) {
        ...
        var username = $("#username").text();
            $.ajax({
                type: "POST", //用POST方式传输
                url: HOST + "index.php/Home/BxWorkDefine/updateNbBdDefine", //目标地址.
                dataType: "json", //数据格式:JSON
                data: {username: username, business_code: business_code, policy_code: policy_code, business_name:business_name,media_type:media_type,insert_date:insert_date},
...
window.actionEvents_check = {
    'click .check': function (e, value, row, index) {
        ...
        $.ajax({
            type: "POST", //用POST方式传输
            url: HOST + "index.php/Home/BxWorkDefine/updateNbBdDefineCheck", //目标地址.
            dataType: "json", //数据格式:JSON
            data: {username: username, business_code: business_code, policy_code: policy_code, business_name:business_name,media_type:media_type,insert_date:insert_date},
            success: function (result) {
                if (result.status == 'success') {
...
function actionFormatter_select(value, row, index) {
    if(row.is_check_policy != "1") {
        ...
        return '<span style="color:rgba(44,173,164,0.72);"><strong>已抽取保单</strong></span>';
    }else if(row.is_check_policy == "1"){
        return '<span style="color:rgba(247,215,64,0.97);"><strong>待抽取保单</strong></span>';
    }
}
