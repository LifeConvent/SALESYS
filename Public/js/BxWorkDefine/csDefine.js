...
window.actionEvents_review = {
    ...
    'click .pass_error': function (e, value, row, index) {
        ...
        $.ajax({
            ...
            dataType: "json", //数据格式:JSON
            //针对于未提交缺陷的系统错误
            data: {username: username,is_pass:'1',business_code: business_code, policy_code: policy_code, business_node:business_node,insert_date:busi_insert_date,result:result,no_pass_reason:no_pass_reason},
            success: function (result) {
                if (result.status == 'success') {
                    debugger;
                    //单行刷新数据
