...
//            $this->assign('username_chinese', $method->getUserCNNameBySql($username));
    ...
    public function dealHomeCount()
    {
        ...
        #####################################################################  保全数据查询  #####################################################################
        #039
        $select_cs = "SELECT SUM(NUM) AS NUM FROM (SELECT COUNT(*) AS NUM FROM TMP_NCS_BX_BQSL_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY INSERT_SYSDATE)";
        $result_rows = oci_parse($conn, $select_cs); // 配置SQL语句，执行SQL
        ...
        $select_cs = "SELECT TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') AS NEW_INSERT_TIME,
                        ...
                        (SELECT * FROM (SELECT TO_CHAR(INSERT_SYSDATE,'YYYY-MM-DD') AS NEW_INSERT_TIME,COUNT(*) AS NUM FROM TMP_NCS_BX_BQSL_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY INSERT_SYSDATE ORDER BY INSERT_SYSDATE DESC) WHERE ROWNUM<8) B
        ...
        #####################################################################  契约数据查询  #####################################################################
        #039
        $select_cs = "SELECT SUM(NUM) AS NUM FROM (SELECT COUNT(*) AS NUM FROM TMP_NCS_BX_NB_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY TO_CHAR(INSERT_SYSDATE,'YYYY-MM-DD'))";
        $result_rows = oci_parse($conn, $select_cs); // 配置SQL语句，执行SQL
        $cs_result = $method->search_long($result_rows);
        ...
        $select_nb = "SELECT TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') AS NEW_INSERT_TIME,
                              ...
                              END) AS NUM
                        FROM TMP_QDSX_TIME A LEFT JOIN
                        (SELECT * FROM (SELECT TO_CHAR(INSERT_SYSDATE,'YYYY-MM-DD') AS NEW_INSERT_TIME,COUNT(*) AS NUM FROM TMP_NCS_BX_NB_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY INSERT_SYSDATE ORDER BY INSERT_SYSDATE DESC) WHERE ROWNUM<8) B
                        ON TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') = B.NEW_INSERT_TIME  ORDER BY TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD')";
        ...
        #####################################################################  理赔数据查询  #####################################################################
        #039
        $select_cs = "SELECT SUM(NUM) AS NUM FROM (SELECT COUNT(*) AS NUM FROM TMP_NCS_BX_LP_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY INSERT_SYSDATE)";
        $result_rows = oci_parse($conn, $select_cs); // 配置SQL语句，执行SQL
        $cs_result = $method->search_long($result_rows);
        ...
        $select_clm = "SELECT TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') AS NEW_INSERT_TIME,
                              ...
                              END) AS NUM
                        FROM TMP_QDSX_TIME A LEFT JOIN
                        (SELECT * FROM (SELECT TO_CHAR(INSERT_SYSDATE,'YYYY-MM-DD') AS NEW_INSERT_TIME,COUNT(*) AS NUM FROM TMP_NCS_BX_LP_BD WHERE INSERT_SYSDATE IS NOT NULL AND NEW_ORGAN_CODE IS NOT NULL GROUP BY INSERT_SYSDATE ORDER BY INSERT_SYSDATE DESC) WHERE ROWNUM<8) B
                        ON TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') = B.NEW_INSERT_TIME  ORDER BY TO_CHAR(A.NEW_INSERT_TIME,'YYYY-MM-DD') ";
