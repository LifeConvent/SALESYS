...
//        dump(json_decode(json_encode($result)));
    ...
    public function getMenuArrayListResult()
    {
        $method = new MethodController();
        $conn = $method->OracleOldDBCon();
        $user_select = "SELECT * FROM TMP_BX_MENU_LIST";
        $result_rows = oci_parse($conn, $user_select); // 配置SQL语句，执行SQL
    ...
    public function getMenuArrayListBx()
    {
        $method = new MethodController();
        $conn = $method->OracleOldDBCon();
        $user_select = "SELECT * FROM TMP_BX_MENU_LIST";
        ...
        $this->assign('menuListBx', $content);
    }
    ...
    public function postModifyMenu()
    {
        ...
        $method = new MethodController();
        $conn = $method->OracleOldDBCon();
        $user_modify = "UPDATE TMP_BX_MENU_LIST SET MENU_NAME = '" . $menu_name . "' , IS_LOCK = '" . $is_lock . "'WHERE MENU_INDEX = '" . $menu_index . "'";
...
//        dump($user_modify);
    ...
    public function postAddMenu()
    {
        ...
        $method = new MethodController();
        $conn = $method->OracleOldDBCon();
        $menu_add = "INSERT INTO TMP_BX_MENU_LIST(MENU_NAME,MENU_INDEX,IS_LOCK,MENU_GRADE,MENU_CODE) 
    ...
    public function getMenuList()
    {
        $method = new MethodController();
        $conn = $method->OracleOldDBCon();
        $user_select = "SELECT * FROM TMP_BX_MENU_LIST ORDER BY MENU_INDEX";
        $result_rows = oci_parse($conn, $user_select); // 配置SQL语句，执行SQL
        $menu_result = $method->search_long($result_rows);
        oci_free_statement($result_rows);
        oci_close($conn);
