    ...
    public function getCsDefine(){
        ...
        if (!empty($queryDateStart)) {
            if (!empty($queryDateEnd)) {
                $where_time_bqsl = " AND TRUNC(BD.INSERT_SYSDATE) BETWEEN to_date('" . $queryDateStart . "','yyyy/mm/dd') AND to_date('" . $queryDateEnd . "','yyyy/mm/dd') ";
            } else {
                $where_time_bqsl = " AND TRUNC(BD.INSERT_SYSDATE) = to_date('" . $queryDateStart . "','yyyy/mm/dd') ";
            }
        } else {
            $where_time_bqsl = " AND TRUNC(BD.INSERT_SYSDATE) = TRUNC(SYSDATE) ";
        }
        ##############################################################  测试数据  ############################################################################
        #$where_time_bqsl = "";
        ##############################################################  测试数据  ############################################################################
...
//            dump($organCode);
            $where_type_fix =  " AND BD.OLD_ORGAN_CODE LIKE '".$organCode[$user_name]."%'";
        }else if((int)$userType==3){
            $where_type_fix = " AND BD.NEW_USER_NAME = '".$user_name."'";
        }
        if(in_array($user_name,$otherUser)){
            $where_type_fix =  " AND BD.OLD_ORGAN_CODE NOT LIKE '8647%'";
        }
        ...
        if((!in_array($user_name,$fuhe_user)&&!in_array($user_name,$clm_user)&&!in_array($user_name,$uw_user))||(int)$userType==1) {
            #033 个人待确认保全受理查询
            $select_bqsl = "SELECT BD.OLD_ORGAN_CODE,
                                       BD.NEW_ORGAN_CODE,
                                       BD.OLD_USER_NAME,
                                       BD.NEW_USER_NAME,
                                       BD.OLD_ORGAN_NAME,
                                       ...
                                       BD.OLD_GET_MONEY,
                                       BD.NEW_GET_MONEY,
                                       TO_CHAR(BD.OLD_INSERT_TIME,'YYYY-MM-DD') AS OLD_INSERT_TIME,
                                       TO_CHAR(BD.NEW_INSERT_TIME,'YYYY-MM-DD') AS NEW_INSERT_TIME,
                                       BD.IS_ACCORDANCE,
                                       TO_CHAR(BD.INSERT_SYSDATE,'YYYY-MM-DD') AS INSERT_SYSDATE,
                                       ...
                                       B.DESCRIPTION
                                   FROM TMP_NCS_BX_BQSL_BD BD
                                   LEFT JOIN TMP_BX_DAYPOST_DESCRIPTION B
                                        ON BD.OLD_ACCEPT_CODE = B.BUSINESS_CODE
                                        AND BD.OLD_POLICY_CODE = B.POLICY_CODE
                                        AND B.BUSINESS_NODE = 'BQSL'
                                        AND TRUNC(B.BUSINESS_DATE) = TRUNC(BD.INSERT_SYSDATE)
